...
def load_model_and_transform(model_name: str, device: torch.device):
    ...
    if model_name == "dinov1":
        ...
        feature_dim = 768 * 2  # CLS + patch avg concatenados → 1536
        print(f"  DINOv1 cargado ✅  feature dim final: {feature_dim}")
        ...
        transform   = _imagenet_transform()
        feature_dim = 768 * 2  # CLS + patch avg → 1536
        print(f"  DINOv2 cargado ✅  feature dim final: {feature_dim}")
        ...
        # Mismo transform ImageNet que DINOv1/v2, no necesitamos el processor de HF
        transform   = _imagenet_transform()
        feature_dim = model.config.hidden_size * 2  # CLS + patch avg -> 1536
        model._num_register_tokens = getattr(model.config, "num_register_tokens", 0)
        print(f"  DINOv3-ViT cargado OK  register tokens: {model._num_register_tokens}  feature dim: {feature_dim}")
        ...
        transform   = _imagenet_transform()
        feature_dim = model.config.hidden_sizes[-1]
        print(f"  DINOv3-ConvNext cargado OK  feature dim: {feature_dim}")
...
def _imagenet_transform() -> transforms.Compose:
    """Transform estándar ImageNet para DINOv1 y DINOv2."""
...
def extract_features(
    ...
    Estrategia por modelo:
      DINOv1       → CLS token + media patch tokens (sin CLS)         → (N, 1536)
      DINOv2       → x_norm_clstoken + media x_norm_patchtokens       → (N, 1536)
      DINOv3-ViT   → CLS token + media patch tokens (sin register)    → (N, 1536)
      DINOv3-ConvNext → pooler_output (global avg pool del último mapa)→ (N, 768)
    """
    all_features = []
...
Dimensiones de salida:
  ...
  dinov3-convnext → (N, 768)   pooler output (global avg pool)
        """
    )
